...
async function sendWhatsAppNotification(
    ...
    //     to: phoneNumber,
    //     type: "template",
    //     message: { templateid: "878529", placeholders: [count] },
    // };
...
export async function completeCheckIn(req: Request, res: Response) {
    try {
        ...
        if (thresholds.includes(count)) {
            ...
            const phoneNumbers = await getRecipientPhones('mhc_checkin'); // DB-managed recipients
